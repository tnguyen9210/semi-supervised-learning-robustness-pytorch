...
from core.objectives import MT
...
class DeepModel(object):
    def __init__(self, args):
        ...
        self.mt_ema_factor = args['mt_ema_factor']
        self.consis_coef = args['mt_consis_coef']
        ...
        self.mt_crit = MT(self.net, self.mt_ema_factor)
    ...
    def update(self, train_lbl, train_unlbl, epoch, logger):
        ...
        for i in range(self.num_iters_per_epoch):
            ...
            # unlbl_x = self.transform(unlbl_x) # transform inside mt_crit
            lbl_logit_y = self.net(lbl_x)
            
            # compute losses
            lbl_loss_y = self.cls_crit(lbl_logit_y, lbl_y)
            
            unlbl_loss_mt = self.mt_crit(unlbl_x, self.net, self.transform)

            # ramp up exp(-5(1 - t)^2)
            coef = self.consis_coef \
                * math.exp(-5*(1 - min(self.it/self.consis_warmup, 1))**2)
            # coef = self.consis_coef
            loss = lbl_loss_y + coef*unlbl_loss_mt
            ...
            self.mt_crit.moving_average(self.net.parameters())
