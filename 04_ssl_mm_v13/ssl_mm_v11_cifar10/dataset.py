...
def load_data(domain, data_dir, img_size, num_iters_per_epoch, batch_size):
    ...
    # load data
    train_unlbl = ImageDatasetUnlbl(data_dir, domain, img_size, ds='train_unlbl_4000', transform=train_transform)
    train_lbl = ImageDataset(data_dir, domain, img_size, ds='train_lbl_4000', transform=train_transform)
    ...
    train_unlbl_sampler = data.RandomSampler(
        train_unlbl, replacement=True, num_samples=num_iters_per_epoch*batch_size)

    train_unlbl = data.DataLoader(
        train_unlbl, batch_size=batch_size, sampler=train_unlbl_sampler, num_workers=16)
    ...
    train_lbl = data.DataLoader(
        train_lbl, batch_size=batch_size, sampler=train_lbl_sampler, num_workers=16)
    ...
    return train_unlbl, train_lbl, dev_lbl, test_lbl
...
class ImageDatasetUnlbl(data.Dataset):
    def __init__(self, data_dir, domain, img_size, ds, transform):
        self.transform = transform
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            ...
            img2 = transforms.functional.to_tensor(img)
