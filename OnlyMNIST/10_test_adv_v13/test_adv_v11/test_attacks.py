...
def test_attacks():
    ...
    # branded 
    # attackers = [
    #     # GradientSignAttack(loss_fn=F.cross_entropy, eps=linf_eps),
    #     # LinfPGDAttack(
    #     #     loss_fn=F.cross_entropy, num_iters=linf_niters, eps=linf_eps, eps_iter=linf_eps_iter),

    #     # GradientAttack(loss_fn=F.cross_entropy, eps=l2_eps),
    #     L2PGDAttack(
    #         loss_fn=F.cross_entropy, num_iters=l2_niters, eps=l2_eps, eps_iter=l2_eps_iter),
    # ]

    # virtual branded
    # attackers = [
    #     VirtualGradientSignAttack(loss_fn=kl_div, eps=linf_eps),
    #     VirtualLinfPGDAttack(
    #         loss_fn=kl_div, num_iters=num_iters, eps=linf_eps, eps_iter=linf_eps_iter),
    #     # VirtualGradientAttack(loss_fn=kl_div, eps=l2_eps)
    #     # VirtualL2PGDAttack(
    #     #     loss_fn=kl_div, num_iters=num_iters, eps=l2_eps, eps_iter=l2_eps_iter),
    # ]
    ...
    for attacker in attackers:
        ...
        for i, batch in enumerate(eval_lbl):
            ...
            # adv_x, losses = attacker.perturb(net, orig_x, orig_y)
            ...
            # adv_logit = net(adv_x)
            # losses = F.cross_entropy(adv_logit, orig_y)
            # print(losses)
            # stop
