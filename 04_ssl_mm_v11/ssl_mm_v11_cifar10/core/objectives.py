...
class MixMatch(nn.Module):
    ...
    def forward(self, unlbl_x, lbl_x, lbl_y, net, transform):
        batch_size = unlbl_x.shape[0]

        # create one lbl_x augment and K unlbl augments
        lbl_x = transform(lbl_x)
        unlbl_x1 = transform(unlbl_x)
        unlbl_x2 = transform(unlbl_x)
